...
def evaluate_model(model):
    ...
    for seq_set in tqdm(QUERY_SETS, desc='Computing query embeddings'):
        tra_embeddings = []
        for set in QUERY_SETS[seq_set]:
            tra_embeddings.append(get_latent_vectors(model, set))
        QUERY_VECTORS.append(tra_embeddings)
    ...
    for i, g_key in enumerate(DATABASE_SETS):
        ...
        for j, q_key in enumerate(QUERY_SETS):
            if g_key != q_key:
                continue
            for k in range(len(QUERY_SETS[q_key])):
                pair_recall, pair_opr = get_recall(i, j, k, g_key, q_key, DATABASE_VECTORS, QUERY_VECTORS, QUERY_SETS)
...
def get_recall(m, n, v, g_key, q_key, database_vectors, query_vectors, query_sets):
    database_output = database_vectors[m]
    queries_output = query_vectors[n][v]
    ...
    for i in range(len(queries_output)):
        ...
        query_details = query_sets[q_key][v][i]
...
if __name__ == "__main__":
    ...
    cfg.LOG_DIR = 'log/'
    cfg.OUTPUT_FILE = cfg.RESULTS_FOLDER + 'model_0811-2' #ARS548_results_5m.txt
    cfg.MODEL_FILENAME = "best_checkpoint.pth.tar"
    cfg.MODEL_FILENAME = '0811-2/best_checkpoint.pth.tar'
